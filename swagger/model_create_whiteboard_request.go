/*
The Confluence Cloud REST API v2

This document describes Confluence's v2 APIs. This is intended to be an iteration on the existing Confluence Cloud REST API with improvements in both endpoint definitions and performance.

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package swagger

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the CreateWhiteboardRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateWhiteboardRequest{}

// CreateWhiteboardRequest struct for CreateWhiteboardRequest
type CreateWhiteboardRequest struct {
	// ID of the space.
	SpaceId string `json:"spaceId"`
	// Title of the whiteboard.
	Title *string `json:"title,omitempty"`
	// The parent content ID of the whiteboard.
	ParentId *string `json:"parentId,omitempty"`
	// Providing a template key will add that template to the new whiteboard.
	TemplateKey *string `json:"templateKey,omitempty"`
	// If templateKey is provided, locale will decide which language the template will be created with. If locale is omitted, the user's locale will be used.
	Locale *string `json:"locale,omitempty"`
}

type _CreateWhiteboardRequest CreateWhiteboardRequest

// NewCreateWhiteboardRequest instantiates a new CreateWhiteboardRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateWhiteboardRequest(spaceId string) *CreateWhiteboardRequest {
	this := CreateWhiteboardRequest{}
	this.SpaceId = spaceId
	return &this
}

// NewCreateWhiteboardRequestWithDefaults instantiates a new CreateWhiteboardRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateWhiteboardRequestWithDefaults() *CreateWhiteboardRequest {
	this := CreateWhiteboardRequest{}
	return &this
}

// GetSpaceId returns the SpaceId field value
func (o *CreateWhiteboardRequest) GetSpaceId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SpaceId
}

// GetSpaceIdOk returns a tuple with the SpaceId field value
// and a boolean to check if the value has been set.
func (o *CreateWhiteboardRequest) GetSpaceIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SpaceId, true
}

// SetSpaceId sets field value
func (o *CreateWhiteboardRequest) SetSpaceId(v string) {
	o.SpaceId = v
}

// GetTitle returns the Title field value if set, zero value otherwise.
func (o *CreateWhiteboardRequest) GetTitle() string {
	if o == nil || IsNil(o.Title) {
		var ret string
		return ret
	}
	return *o.Title
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateWhiteboardRequest) GetTitleOk() (*string, bool) {
	if o == nil || IsNil(o.Title) {
		return nil, false
	}
	return o.Title, true
}

// HasTitle returns a boolean if a field has been set.
func (o *CreateWhiteboardRequest) HasTitle() bool {
	if o != nil && !IsNil(o.Title) {
		return true
	}

	return false
}

// SetTitle gets a reference to the given string and assigns it to the Title field.
func (o *CreateWhiteboardRequest) SetTitle(v string) {
	o.Title = &v
}

// GetParentId returns the ParentId field value if set, zero value otherwise.
func (o *CreateWhiteboardRequest) GetParentId() string {
	if o == nil || IsNil(o.ParentId) {
		var ret string
		return ret
	}
	return *o.ParentId
}

// GetParentIdOk returns a tuple with the ParentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateWhiteboardRequest) GetParentIdOk() (*string, bool) {
	if o == nil || IsNil(o.ParentId) {
		return nil, false
	}
	return o.ParentId, true
}

// HasParentId returns a boolean if a field has been set.
func (o *CreateWhiteboardRequest) HasParentId() bool {
	if o != nil && !IsNil(o.ParentId) {
		return true
	}

	return false
}

// SetParentId gets a reference to the given string and assigns it to the ParentId field.
func (o *CreateWhiteboardRequest) SetParentId(v string) {
	o.ParentId = &v
}

// GetTemplateKey returns the TemplateKey field value if set, zero value otherwise.
func (o *CreateWhiteboardRequest) GetTemplateKey() string {
	if o == nil || IsNil(o.TemplateKey) {
		var ret string
		return ret
	}
	return *o.TemplateKey
}

// GetTemplateKeyOk returns a tuple with the TemplateKey field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateWhiteboardRequest) GetTemplateKeyOk() (*string, bool) {
	if o == nil || IsNil(o.TemplateKey) {
		return nil, false
	}
	return o.TemplateKey, true
}

// HasTemplateKey returns a boolean if a field has been set.
func (o *CreateWhiteboardRequest) HasTemplateKey() bool {
	if o != nil && !IsNil(o.TemplateKey) {
		return true
	}

	return false
}

// SetTemplateKey gets a reference to the given string and assigns it to the TemplateKey field.
func (o *CreateWhiteboardRequest) SetTemplateKey(v string) {
	o.TemplateKey = &v
}

// GetLocale returns the Locale field value if set, zero value otherwise.
func (o *CreateWhiteboardRequest) GetLocale() string {
	if o == nil || IsNil(o.Locale) {
		var ret string
		return ret
	}
	return *o.Locale
}

// GetLocaleOk returns a tuple with the Locale field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateWhiteboardRequest) GetLocaleOk() (*string, bool) {
	if o == nil || IsNil(o.Locale) {
		return nil, false
	}
	return o.Locale, true
}

// HasLocale returns a boolean if a field has been set.
func (o *CreateWhiteboardRequest) HasLocale() bool {
	if o != nil && !IsNil(o.Locale) {
		return true
	}

	return false
}

// SetLocale gets a reference to the given string and assigns it to the Locale field.
func (o *CreateWhiteboardRequest) SetLocale(v string) {
	o.Locale = &v
}

func (o CreateWhiteboardRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateWhiteboardRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["spaceId"] = o.SpaceId
	if !IsNil(o.Title) {
		toSerialize["title"] = o.Title
	}
	if !IsNil(o.ParentId) {
		toSerialize["parentId"] = o.ParentId
	}
	if !IsNil(o.TemplateKey) {
		toSerialize["templateKey"] = o.TemplateKey
	}
	if !IsNil(o.Locale) {
		toSerialize["locale"] = o.Locale
	}
	return toSerialize, nil
}

func (o *CreateWhiteboardRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"spaceId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCreateWhiteboardRequest := _CreateWhiteboardRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varCreateWhiteboardRequest)

	if err != nil {
		return err
	}

	*o = CreateWhiteboardRequest(varCreateWhiteboardRequest)

	return err
}

type NullableCreateWhiteboardRequest struct {
	value *CreateWhiteboardRequest
	isSet bool
}

func (v NullableCreateWhiteboardRequest) Get() *CreateWhiteboardRequest {
	return v.value
}

func (v *NullableCreateWhiteboardRequest) Set(val *CreateWhiteboardRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateWhiteboardRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateWhiteboardRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateWhiteboardRequest(val *CreateWhiteboardRequest) *NullableCreateWhiteboardRequest {
	return &NullableCreateWhiteboardRequest{value: val, isSet: true}
}

func (v NullableCreateWhiteboardRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateWhiteboardRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


